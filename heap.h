...
void heap_sift_up(void *data, size_t index, const cmp_t cmp, const size_t width) {
	while (index > 0) {
...
void heap_sift_down(void *data, const size_t len, size_t index, const cmp_t cmp, const size_t width) {
	while (true) {
...
void heap_update_key(void *data, size_t len, size_t index, void *new_item, cmp_t cmp, size_t width) {
	...
	if (comp > 0) {
		heap_sift_up(data, index, cmp, width);
	}

	if (comp < 0) {
		heap_sift_down(data, len, index, cmp, width);
	}
}

void heap_push(void *data, size_t len, const cmp_t cmp, size_t width) {
	heap_sift_up(data, len - 1, cmp, width);
}
...
void heap_pop(void *data, size_t len, const cmp_t cmp, size_t width) {
	...
	heap_sift_down(data, new_len, 0, cmp, width);
}

void heap_build(void *data, const size_t len, const cmp_t cmp, const size_t width) {
	for (size_t i = len / 2 - 1; i <= len / 2 - 1; i -= 1) {
		heap_sift_down(data, len, i, cmp, width);
	}
}
